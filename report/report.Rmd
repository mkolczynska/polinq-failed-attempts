...
# ess7_full <- import_rounds(7, ess_email = "email@email") %>%
#   select(idno, cntry, pspwght, wrkorg, wrkprty, pbldmn, contplt, 
#          sgnptit, bctprd, vote, gndr, eisced, agea, polintr) %>%
#   recode_missings()
...
# Data source: https://dbk.gesis.org/dbksearch/sdesc2.asp?no=6670
...
```{r polpart-levels-inequality, include=FALSE}
...
ess7_full <- left_join(ess7[,c("cntry", "pspwght", "polint", "nobs", ess_part_vars)], weight, by = c("cntry"))

ess7_full$polpart_z <- rowSums(mapply(`*`,ess7_full[,5:9],ess7_full[,10:14]), na.rm = TRUE)
ess7_full$polpart_z <- round(ess7_full$polpart_z,3)

ess7_full$polpart_ln <- rowSums(mapply(`*`,ess7_full[,5:9],ess7_full[,15:19]), na.rm = TRUE)
ess7_full$polpart_ln <- round(ess7_full$polpart_ln,3)

ess7_full$polpart_rank <- rowSums(mapply(`*`,ess7_full[,5:9],ess7_full[,20:24]), na.rm = TRUE)
...
ess_cntrydata <- ess7_full %>%  group_by(cntry) %>% 
  summarise_at(c(ess_vars, "nobs"), funs(weighted.mean(., pspwght)), na.rm = TRUE)
...
issp_full <- left_join(issp2014[,c("cntry", "WEIGHT", "polint", "nobs", issp_part_vars)], issp_weight, by = c("cntry"))

issp_full$polpart_z <- rowSums(mapply(`*`,issp_full[,5:9],issp_full[,10:14]), na.rm = TRUE)
issp_full$polpart_z <- round(issp_full$polpart_z,3)

issp_full$polpart_ln <- rowSums(mapply(`*`,issp_full[,5:9],issp_full[,15:19]), na.rm = TRUE)
issp_full$polpart_ln <- round(issp_full$polpart_ln,3)

issp_full$polpart_rank <- rowSums(mapply(`*`,issp_full[,5:9],issp_full[,20:24]), na.rm = TRUE)
...
issp_cntrydata <- issp_full %>% group_by(cntry) %>% 
  summarise_at(c(issp_vars, "nobs"), funs(weighted.mean(., WEIGHT)), na.rm = TRUE)
...
# Attempt 0: Country levels of participation in single activities
...
In both cases samples have been restricted to respondents aged 18-65. Appropriate case weights -- `pspwgh` in ESS and `WEIGHT` in ISSP -- were applied. 
...
ess_issp_cntrydata %>%
  mutate(indicator = ifelse(indicator == "demo_rally", "demo", indicator)) %>%
  filter(indicator %in% c("party", "demo", "contact", "petition", "boycott")) %>%
  gather(item, val, c(2, 4)) %>%
  unite(si, survey, item) %>%
  spread(si, val) %>%
  drop_na() %>%
  mutate(prop = (ESS_value * ESS_nobs + ISSP_value * ISSP_nobs) / (ESS_nobs + ISSP_nobs),
         se = sqrt(prop*(1-prop)*( (1/ESS_nobs + 1/ISSP_nobs))),
         z = abs((ESS_value - ISSP_value) / se),
         flag = as.factor(abs(z) > 1.96)) %>%
  ggplot(., aes(x = ESS_value, y = ISSP_value, col = indicator)) + 
  geom_point(aes(size = z), alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, col = "gray50") +
  geom_text(aes(label=cntry), size=3, hjust = 0, nudge_x = 0.009) +
  labs(caption = 
...
where $p_{i,j}$ is the level of participation in activity *i* in country *j*. Figure \@ref(fig:weights) shows how the value of the weight depends on the level of participation. In the analyses that follow I decided to drop the reciprocal of the square root because of the extremely high weights it would assign to rare activities.
...
```{r cntry-ranks, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.cap="Political participation ranks: ESS/7 and ISSP/2014.", fig.pos="H", fig.width=7, fig.height=7}
